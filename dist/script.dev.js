...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var carousel = new bootstrap.Carousel(document.getElementById('recipeCarousel'), {
    interval: 2000,
    // Set your desired interval in milliseconds
    pause: 'hover' // Pause on mouse enter

  }); // Clone items for seamless autoplay

  var items = document.querySelectorAll('.sponsor .carousel .carousel-item');
  ...
  var articleNavLinks = document.querySelectorAll(".article .navbar li");
  var photoCount = 5;
  var videoDuration = 105;
  ...
  articleNavLinks.forEach(function (link) {
    link.addEventListener("click", function (event) {
      ...
      cards.forEach(function (card) {
        var countElement = card.querySelector(".count-element");
      ...
      if (link.textContent.trim() === "Foto") {
        cards.forEach(function (card) {
          var newCountElement = document.createElement("div");
          newCountElement.className = "count-element";
          var fotoIcon = createIconElement("./public/img/foto.svg");
          newCountElement.appendChild(fotoIcon);
          var countText = document.createElement("span");
          countText.textContent = "".concat(photoCount, " Photos");
          newCountElement.appendChild(countText); // Position the count element at the top right of the image
        ...
        cards.forEach(function (card) {
          var newCountElement = document.createElement("div");
          newCountElement.className = "count-element";
          var videoIcon = createIconElement("./public/img/video.svg");
          newCountElement.appendChild(videoIcon);
          var countText = document.createElement("span");
          countText.textContent = "".concat(timeString);
          newCountElement.appendChild(countText); // Position the count element at the top right of the image
  ...
  var menuIcon = document.querySelector('[data-bs-toggle="collapse"]');
